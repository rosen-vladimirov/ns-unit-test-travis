...
function NativeScriptLauncher(baseBrowserDecorator, logger, config, args, emitter, executor) {
	...
	emitter.on('browser_register', function(browser) {
		...
		console.log("### karma-nativescript-launcher ### browser_register will schedule execution");
		self.markCaptured();
	...
	self.start = function(url) {
		...
		console.log("### karma-nativescript-launcher ### start will send message to parent process");
