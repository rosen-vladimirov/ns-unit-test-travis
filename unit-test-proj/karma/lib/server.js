...
var Server = function (cliOptions, done) {
  ...
  console.log("### karma ### constructor, cliOptions: ", cliOptions);
  ...
  console.log("### karma ### constructor, config: ", config);
...
Server.prototype._start = function (config, launcher, preprocess, fileList,
  ...
  console.log("### karma ### start called, config = ", config);
  var self = this
  if (config.detached) {
  ...
  if (config.singleRun) {
    ...
    self.on('run_complete', function (browsers, results) {
      console.log("### karma ###: RUN COMPLETE EVENT!!!!!")
  ...
  var disconnectBrowsers = function (code) {
    console.log("### karma ###: disconnectBrowsers, code = ", code);
